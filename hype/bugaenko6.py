...
class Bugaenko6Manifold(Manifold):
    ...
    def ldot(u, v, keepdim=False):
        ...
        -(1+np.sqrt(2))x0^2+x1^2+..."""
        uv = u * v
        uv.narrow(-1, 0, 1).mul_(-1-np.sqrt(2))
        return th.sum(uv, dim=-1, keepdim=keepdim)

    def to_poincare_ball(self, u, u_int_matrix):
        M = u_int_matrix[:,0,:,:] + np.sqrt(2)*u_int_matrix[:,1,:,:]
        #N = u_int_norm[:,0] + np.sqrt(2)*u_int_norm[:,1]
        u = th.matmul(M, u.unsqueeze(-1)).squeeze(-1)
        #u = u/N
        d = u.size(-1) - 1
        b = np.sqrt(1+np.sqrt(2))
        return u.narrow(-1, 1, d) / (b*u.narrow(-1, 0, 1) + 1)
    ...
    def init_weights_int_norm(self, w):
        U = th.tensor([1])
        w.data.zero_()
        w.data.add(U)
    ...
    def rgrad(self, p, d_p):
        ...
        if d_p.is_sparse:
            ...
            u = d_p
            x = p
        u.narrow(-1, 0, 1).mul_(-1/(1+np.sqrt(2)))
...
def matmul_constructible(L,R):
    '''Takes two pair of matrices ot type (...,2,x,x)
    '''
    L0 = L.narrow(-3,0,1); R0 = R.narrow(-3,0,1)
    L1 = L.narrow(-3,1,1); R1 = R.narrow(-3,1,1)

    LR0 = th.matmul(L0,R0)+2*th.matmul(L1,R1)
    LR1 = th.matmul(L1,R0)+th.matmul(L0,R1)

    return th.cat((LR0, LR1), -3)
...
class GroupRieDistance(Function):
    ...
    def forward(self, u, u_int_matrix, v, v_int_matrix, g, AvOverflow = False, myeps1 = 1e-8 ,myeps2 = 1e-16, decompose_factor = 25):
        ...
        gv_int_matrix = matmul_constructible(g,v_int_matrix)
        ...
        Q = matmul_constructible(u_int_matrix1, gv_int_matrix1)\
            +matmul_constructible(u_int_matrix1, gv_int_matrix2)\
            +matmul_constructible(u_int_matrix2, gv_int_matrix1)\
            +matmul_constructible(u_int_matrix2, gv_int_matrix2)
                
        #N[i][j] = matmul_constructible(u_int_norm[i][j], v_int_norm[i][j])

        RQ = -Q.narrow(-3,0,1) - np.sqrt(2)*Q.narrow(-3,1,1); RQ.squeeze_(-3)
        max_coef = RQ.max(-1, keepdim=True)[0].max(-2,keepdim=True)[0]
        #print(max_coef)
        self.hatRQ = th.div(RQ,max_coef)
        #self.RN = N[:,:,0] + np.sqrt(2)*N[:,:,1]
        #self.RQ_unitary = RQ/RN
        #d_c = u^T(RQ)v
        d_c = th.matmul(u.unsqueeze(-1).transpose(-2,-1), th.matmul(self.hatRQ, v.unsqueeze(-1))).squeeze(-1).squeeze(-1)#cpu float       
        #print('d_c', d_c[5,5])
    ...
    def backward(self, g):
        ...
        uupfrac = th.matmul(self.hatRQ, v.unsqueeze(-1)).squeeze(-1)
        vupfrac = th.matmul(self.hatRQ.transpose(-2,-1), u.unsqueeze(-1)).squeeze(-1)
