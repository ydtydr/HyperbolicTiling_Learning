...
class LorentzManifold(Manifold):
    ...
    @staticmethod
    def ldot(u, v, a=1, keepdim=False):
        """Lorentzian Scalar Product"""
        uv = u * v
        uv.narrow(-1, 0, 1).mul_(-a)
        return th.sum(uv, dim=-1, keepdim=keepdim)
    '''
    def const_ldot(u, v, a=1, keepdim=False):
        uv = u * v
        uv[0] = (-a)*uv[0]
        return uv.sum()'''

    def const_ldot(u, v, a=1, keepdim=False):
        uv0 = u[0]*v[0] + 2*u[1]*v[1]
        uv1 = u[0]*v[1] + u[1]*v[0]
        x = (-a[0])*uv0[0] + 2*(-a[1])*uv1[0]
        y = (-a[1])*uv0[0] + (-a[0])*uv1[0]
        uv0[0] = x
        uv1[0] = y
        return uv0.sum(), uv1.sum() 
