...
from hype.hyperbolicspace import move_to_hyperboloid


#in this model coordinates g[0:d] are coordinates in halfspace model (the last is positive). 
#g[d:-3] are translations parallel to the boundary, g[-2] = 0, g[-1] is the exponens of the dilatation 2*. 
...
class HalfspaceRieManifold(Manifold):
    ...
    def hyperboloid_to_halfspace(self,x):
        '''in hyperboloid the first coordinate is time-like, 
        in halfspace the last mast be positive'''
        x = th.div(x,x[0])
        x = x[1:]
        #inversion of radious 2 with respect to point (0,...,0,-1)
        x[-1] = x[-1]+1
        x = th.div(4*x,th.sum(x*x))
        x[-1] = x[-1]-2
        return x
    ...
    def normalize_halfspace(self,g):
        y = th.zeros(g.size())
        n = (g.size(0)-1)//2
    ...
    def init_weights(self, w, irange=1e-5, faraway=0):
        d = (w.size(-1)-1)//2
        ...
        if not faraway:
            ...
            g = th.zeros(w.size(-1))
            x = faraway*th.randn(d+1) 
            x = move_to_hyperboloid(x)

            g[:d] = self.hyperboloid_to_halfspace(x)

            g = self.normalize_halfspace(g)
            w.data[...,d:].zero_()
            w.data[...,d:].add_(g[d:])
...
class HalfspaceRieDistance(Function):
    ...
    def backward(self, g):
        ...
        if not self.AvOverflow:
            ...
            assert th.isnan(auxli_term1).max() == 0, "at1 includes NaNs"
            assert th.isnan(auxli_term2).max() == 0, "at2 includes NaNs"

            #print(auxli_term1)

            gu[..., :d - 1] = auxli_term1.unsqueeze(-1).expand_as(u[...,:d-1]) * auxli_term2[...,:d-1]#m*n*(d-1)

            assert th.isnan(gu).max() == 0, "gu1 includes NaNs"
